...
var apiGraphqlapiGraphQLAPIIdOutput = process.env.API_BOYAKIDENDOGQL_GRAPHQLAPIIDOUTPUT
var apiGraphqlapiGraphQLAPIEndpointOutput = process.env.API_BOYAKIDENDOGQL_GRAPHQLAPIENDPOINTOUTPUT
...
exports.handler = async (event, context, callback) => {
    ...
    if ('AWS_EXECUTION_ENV' in process.env && process.env.AWS_EXECUTION_ENV.startsWith('AWS_Lambda_')) {
        ...
        // for local mock
        env = {
            API_BOYAKIDENDOGQL_GRAPHQLAPIENDPOINTOUTPUT: 'http://localhost:20002/graphql',
            REGION: 'us-east-1',
        }
        graphql_auth = {
            type: "AWS_IAM",
    ...
    if (!graphqlClient) {
        graphqlClient = new AWSAppSyncClient({
            url: env.API_BOYAKIDENDOGQL_GRAPHQLAPIENDPOINTOUTPUT,
            region: env.REGION,
            auth: graphql_auth,
            disableOffline: true,
        });
